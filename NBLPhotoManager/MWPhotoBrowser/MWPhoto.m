...
+ (MWPhoto *)photoWithURL:(NSURL *)url alAsset:(ALAsset *)alAsset {
    return [[MWPhoto alloc] initWithURL:url alAsset:alAsset];
}
...
- (id)initWithURL:(NSURL *)url alAsset:(ALAsset *)alAsset{
    if ((self = [super init])) {
        self.photoURL = url;
        self.alAsset = alAsset;
        NSString *photoType = [alAsset valueForProperty:ALAssetPropertyType];
        if ([photoType isEqualToString:ALAssetTypeVideo]) {
            self.isVideo = YES;
        } else {
            self.isVideo = NO;
        }
