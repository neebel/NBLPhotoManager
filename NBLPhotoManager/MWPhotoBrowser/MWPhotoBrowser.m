...
- (void)performLayout {
    ...
    self.confirmButton.frame = CGRectMake(0, 0, 70, 44);
    self.confirmButton.titleLabel.font = [UIFont systemFontOfSize:15];
    [self.confirmButton setTitle:@"确定(0)" forState:UIControlStateNormal];
    [self.confirmButton setTitleColor:[UIColor colorWithRed:0.08 green:0.49 blue:0.98 alpha:1.0]
                             forState:UIControlStateNormal];
    [self.confirmButton addTarget:self action:@selector(doneButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    _doneButton = [[UIBarButtonItem alloc] initWithCustomView:self.confirmButton];
    ...
    UIBarButtonItem *rightFixedSpace = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace
                                                                                    target:nil
                                                                                    action:nil];
    rightFixedSpace.width = - 20;
    self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects:rightFixedSpace, _doneButton, nil];
...
- (BOOL)checkPhotoIsEnough:(BOOL)selected
{
    NSInteger nextCount;
    if (selected) {
        nextCount = self.selectedPhotoCount + 1;
    } else {
        nextCount = self.selectedPhotoCount - 1;
    }
...
{
    if ([self.delegate respondsToSelector:@selector(photoBrowserSelectedPhotoCount:)]) {
        self.selectedPhotoCount = [self.delegate photoBrowserSelectedPhotoCount:self];
    }
    
    if (((MWPhoto *)[self photoAtIndex:self.currentIndex]).isVideo && nil == _gridController) {
        [self.confirmButton setTitle:@"确定" forState:UIControlStateNormal];
    } else {
        [self.confirmButton setTitle:[NSString stringWithFormat:@"%@(%@)",@"确定",@(self.selectedPhotoCount).stringValue] forState:UIControlStateNormal];
    }
}
...
- (BOOL)setPhotoSelected:(BOOL)selected atIndex:(NSUInteger)index {
    if (_displaySelectionButtons) {
        ...
        if ([self checkPhotoIsEnough:selected]) {
...
- (void)tilePages {
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add selected button
            if (self.displaySelectionButtons && !photo.isVideo) {
...
- (void)selectedButtonTapped:(id)sender {
    ...
    if (index != NSUIntegerMax) {
        BOOL success = [self setPhotoSelected:!selectedButton.selected atIndex:index];
        if (success) {
...
- (void)doneButtonPressed:(id)sender {
    ...
    if (_doneButton) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowserDidFinishModalPresentation: videoIndex:)]) {
            // Call delegate method and let them dismiss us
            if ([_confirmButton.titleLabel.text isEqualToString:@"确定"]) {
                [_delegate photoBrowserDidFinishModalPresentation:self videoIndex:self.currentIndex];//选择的是视频
            } else {
                [_delegate photoBrowserDidFinishModalPresentation:self videoIndex:-1];//选择的是照片
            }
